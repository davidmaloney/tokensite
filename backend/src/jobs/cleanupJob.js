...
export async function runCleanup() {
 return dbWriteQueue.push(() => {
   ...
   const unpaidResult = db.prepare(
     "DELETE FROM pages WHERE status = 'inactive' AND expires_at IS NULL AND created_at < ? AND id NOT IN (SELECT page_id FROM transactions WHERE confirmed = 1)"
   ).run(unpaidCutoff);
   ...
   const softDeleteResult = db.prepare(
     "UPDATE pages SET status = 'inactive', soft_deleted_at = ?, updated_at = ? WHERE soft_deleted_at IS NULL AND expires_at IS NOT NULL AND expires_at < ?"
   ...
   const toDelete = db.prepare(
     "SELECT id, slug, content_json FROM pages WHERE soft_deleted_at IS NOT NULL AND soft_deleted_at < ?"

  ...
  try {
    ...
    if (!page || page.soft_deleted_at) {
      return res.status(404).send(`
        ...
        <div style="font-size:13px;margin-top:8px"><a href="https://${process.env.DOMAIN}" style="color:#9945FF;text-decoration:none">Create your own →</a></div>
        </div></body></html>`);
    }
    ...
    if (page.status !== "active") {
      return res.status(200).send(`
        <!DOCTYPE html><html><head><meta charset="UTF-8">
        <title>Page Inactive</title>
        <style>body{background:#0d0d0d;color:#666;font-family:system-ui;display:flex;align-items:center;justify-content:center;min-height:100vh;text-align:center}</style>
        </head><body><div><div style="font-size:36px;margin-bottom:12px">💤</div>
        <div style="font-size:16px">This page is currently inactive</div>
        <div style="font-size:13px;margin-top:8px;color:#555">The owner needs to renew their subscription</div>
        <div style="font-size:12px;margin-top:12px"><a href="https://${process.env.DOMAIN}" style="color:#9945FF;text-decoration:none">TokenSite</a></div>
        </div></body></html>`);
    }
